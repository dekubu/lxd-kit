...
module LXDKit
  ...
  class Command
    ...
    Failed = Class.new(LXDKit::StandardError)
    ...
    def environment_hash
      (LXDKit.config.default_env || {}).merge(options[:env] || {})
    end
    ...
    def umask(&_block)
      return yield unless LXDKit.config.umask
      "umask #{LXDKit.config.umask} && #{yield}"
    end
    ...
    def to_s
      if should_map?
        [LXDKit.config.command_map[command.to_sym], *Array(args)].join(' ')
      else
        command.to_s
      end
    end
    ...
    def log_reader_deprecation(stream)
      LXDKit.config.deprecation_logger.log(
