module LXDKit
  ...
  module Formatter
    ...
    class Pretty < Abstract
      ...
      def write(obj)
        if obj.kind_of?(LXDKit::LogMessage)
          write_message(obj.verbosity, obj.to_s)
        else
          raise "write only supports formatting LXDKit::LogMessage, called with #{obj.class}: #{obj.inspect}"
        end
      end
