module LXDKit
  ...
  class Configuration
    ...
    def default_runner_config=(config_hash)
      ...
      LXDKit.config.default_runner = config.delete(:in) if config[:in]
      @default_runner_config = config.merge(in: LXDKit.config.default_runner)
    end
    ...
    # Tell LXDKit to use the specified `formatter` for stdout. The formatter
    # can be the name of a built-in LXDKit formatter, like `:pretty`, a
    # formatter class, like `LXDKit::Formatter::Pretty`, or a custom formatter
    ...
    # Is equivalent to:
    #
    #   config.output = LXDKit::Formatter::Pretty.new($stdout)
    #
    ...
    def command_map
      @command_map ||= LXDKit::CommandMap.new
    end

    def command_map=(value)
      @command_map = LXDKit::CommandMap.new(value)
    end
    ...
    def formatter_class(symbol)
      ...
      found = LXDKit::Formatter.constants.find do |const|
        const.to_s.downcase == name
      end
      fail NameError, %Q{Unrecognized LXDKit::Formatter "#{symbol}"} if found.nil?
      LXDKit::Formatter.const_get(found)
    end
